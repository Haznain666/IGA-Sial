...
export default function Equipment() {
  ...
  const items = availableProducts.filter((p) => p.kind === 'equipment').slice(0, 9)
            ...
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex">
                {slides.map((group, slideIndex) => (
                  <div key={`equipment-slide-${slideIndex}`} className="min-w-0 flex-[0_0_100%] px-0.5">
                    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                      {group.map((item) => (
                        <ProductCard
                          ...
                          product={item}
                          footer={
                            <button onClick={() => sponsorSingle(item.id)} className="btn-gold btn-md w-full">

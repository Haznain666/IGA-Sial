...
// Home "Meet our herd" carousel — live products, up to five available animals.
export default function Herd() {
  const { availableProducts, loading } = useApp()
  const animals = availableProducts.slice(0, 5)
    ...
    <section id="herd" className="scroll-mt-20 bg-cream py-16 sm:py-28">
      <div className="container-x">
        <div className="flex flex-col items-start justify-between gap-5 sm:flex-row sm:items-end">
          <SectionHeading
            align="left"
            eyebrow="Available for donation"
            title="Meet our herd"
            intro="Each animal has a name, a story, and a family waiting. Choose the one that speaks to you."
            className="!mx-0"
          />
        ...
        {loading ? (
          ...
          <div className="mt-10 flex flex-col items-center rounded-3xl border border-dashed border-brand-200 bg-white/60 px-6 py-16 text-center">
            ...
            <h3 className="mt-4 font-heading text-lg font-semibold text-pine">
              Every cow has found a home
            </h3>
            <p className="mt-2 max-w-sm text-sm text-ink/60">
              All animals are currently reserved or donated. Please check back soon.
            </p>
          </div>
        ) : (

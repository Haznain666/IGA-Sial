...
export default function ManageProduct() {
  const { livestock, equipment, settings, addProduct, updateProduct, deleteProduct, statusOf, hasOpenSponsorships, loading } = useApp()
          ...
          <div className="py-12">
            <div className="container-x">
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-3 text-ink/70">Loading products…</span>
              </div>
            </div>
          </div>
        ) : list.length === 0 ? (
...
function ProductEditor({ kind, product, collectOwnerInfo, fxRates, onClose, onSave }) {
  ...
  const submit = async (ev) => {
    ...
    // mark busy so UI disables controls and prevents double submissions
    setBusy(true)
    try {
      if (isEquipment) {
        await onSave({ ...common, warranty: form.warranty.trim(), lifeSpan: form.lifeSpan.trim() })
        return
      }
      ...
      // make sure busy is cleared after the request finishes (success or failure)
      setBusy(false)
    }
  }
      ...
      footer={
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" form="product-form" className="btn-primary btn-md" disabled={busy}>
            {busy ? (
              <>
                <svg className="h-4 w-4 animate-spin -ml-1 mr-2" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                </svg>
                {product ? 'Saving...' : isEquipment ? 'Adding...' : 'Adding...'}
              </>
            ) : (
